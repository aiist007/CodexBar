...
CodexBar 是一款轻量级的 macOS 14+ 菜单栏应用，能够实时显示 Codex、Claude、Cursor、Gemini、Antigravity、Droid (Factory)、Copilot、z.ai、Kimi、Kimi K2、Kiro、Vertex AI、Augment、Amp、JetBrains AI 和 **Nvidia** 的使用限制（支持会话限额和周限额，视具体提供商而定），并显示各个时间窗口的重置时间。每个服务提供商都有独立的图标（也支持"合并图标"模式）；你可以通过设置面板启用你所使用的服务。该应用没有 Dock 图标，界面极简，在菜单栏中通过动态进度条图标展示状态。

> **注意**：此版本已移除 WidgetKit 小组件功能，专注于菜单栏核心体验。
...
### Homebrew
```bash
brew install --cask aiist007/tap/codexbar
```

### Linux (仅限 CLI)
```bash
brew install aiist007/tap/codexbar
```
...
- [Claude](docs/claude.md) — OAuth API 或浏览器 Cookie（带 CLI PTY 备选方案）；支持会话和每周使用量。**支持多账户切换**。
- [Cursor](docs/cursor.md) — 通过浏览器会话 Cookie 获取订阅方案、使用量及账单重置信息。
- [Gemini](docs/gemini.md) — 使用 Gemini CLI 凭据的 OAuth 配额 API（不使用浏览器 Cookie）。
- [Antigravity](docs/antigravity.md) — 本地语言服务器探测（实验性）；无需外部认证。**支持 OAuth 配额探测**。
...
- **[Nvidia](docs/nvidia.md)** — **新增** 支持 Nvidia NGC 使用量监控。
- [OpenCode](docs/opencode.md) — **增强** 改进了 Cookie 导入和设置管理。
...
- **多账户支持**：支持为 Claude 等提供商配置多个账户并快速切换。
- 会话和每周计量表，带有重置倒计时。
...
- **电池优化**：改进了 WebView 缓存管理，减少电池消耗。
- 隐私优先：默认在本地进行数据解析；浏览器 Cookie 为可选启用且仅用于复用（不存储密码）。

## 此版本的特别改进

### 🗑️ 移除的功能
- **WidgetKit 小组件**：已完全移除小组件功能，专注于菜单栏核心体验，减少资源占用。

### ✨ 新增功能
- **多账户支持**：Claude 等提供商现在支持配置多个账户，可在菜单中快速切换。
- **Nvidia 提供商**：新增对 Nvidia NGC 平台的使用量监控支持。
- **电池消耗诊断**：添加了电池使用诊断测试，优化能耗表现。
- **增强的 WebView 缓存**：改进了 OpenAI Dashboard WebView 的缓存管理，提升性能并减少电池消耗。

### 🔧 改进
- **Antigravity**：支持 OAuth 配额探测，提供更准确的使用量数据。
- **OpenCode**：改进了 Cookie 导入流程和设置管理。
- **Gemini**：增强了状态探测功能。
- **Token 账户管理**：改进了 Token 账户的 CLI 支持和数据管理。
...
- **中文使用说明书**: [docs/USER_MANUAL.md](docs/USER_MANUAL.md)

## 快速开始（开发）
- 克隆仓库并在 Xcode 中打开，或直接运行脚本。
- 启动一次，然后在 Settings → Providers 中切换提供商。
- 安装并登录你依赖的服务源（CLI、浏览器 Cookie 或 OAuth）。
- 可选：设置 OpenAI Cookie（自动或手动）以获取 Codex 仪表盘额外数据。
...
## 相关项目
- ✂️ [Trimmy](https://github.com/steipete/Trimmy) — "粘贴一次，运行一次。" 将多行 shell 代码片段扁平化，使其可以粘贴并运行。
- 🧳 [MCPorter](https://mcporter.dev) — 用于 Model Context Protocol 服务器的 TypeScript 工具包 + CLI。
- 🧿 [oracle](https://askoracle.dev) — 当你卡住时询问 oracle。使用自定义上下文和文件调用 GPT-5 Pro。

## 寻找 Windows 版本？
- [Win-CodexBar](https://github.com/Finesssee/Win-CodexBar)

## 致谢
灵感来自 [ccusage](https://github.com/ryoppippi/ccusage) (MIT)，特别是成本使用跟踪功能。
...
**此版本维护者**: [aiist007](https://github.com/aiist007)  
**Fork 地址**: https://github.com/aiist007/CodexBar  
**主要改动**: 移除小组件功能，添加多账户支持，新增 Nvidia 提供商，优化电池使用
